...
const char * const BITCOIN_CONF_FILENAME = "ands.conf";
const char * const BITCOIN_PID_FILENAME = "andsd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "ands" is a composite category enabling all Anodos-related debug output
                if(ptrCategory->count(std::string("ands"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AnodosCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AnodosCore
    // Mac: ~/Library/Application Support/AnodosCore
    // Unix: ~/.andscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AnodosCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AnodosCore";
#else
    // Unix
    return pathRet / ".andscore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty ands.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override ands.conf
