...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in ands.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("ands.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Anodos features */
    { "ands",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "ands",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "ands",               "gobject",                &gobject,                true,  {} },
    { "ands",               "voteraw",                &voteraw,                true,  {} },
